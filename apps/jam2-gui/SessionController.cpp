...
private:
    ...
    std::uint64_t nextBarTarget(const jam2::EngineSnapshot& snapshot) const
    {
    ...
    void handleConsoleCommand(const QString& line)
    {
        ...
        if (command == QStringLiteral("bpm") && parts.size() == 2) {
            ...
            if (action == QStringLiteral("sync") && parts.size() == 3 &&
                ...
                const std::uint64_t target = nextBarTarget(snapshot);
                ...
                engineCommand.transport_action = jam2::EngineTransportAction::TrackRestart;
                engineCommand.transport_target_frame = target;
                engineCommand.transport_musical_frame = target;
