...
struct Jam2RuntimeHost {
    ...
    bool submitPeerUpdate(const std::vector<std::string>& peers);
    std::optional<std::vector<std::string>> takePeerUpdate();
...
private:
    ...
    std::optional<std::vector<std::string>> peer_update_;
