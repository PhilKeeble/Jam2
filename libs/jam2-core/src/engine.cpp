    ...
    void run() noexcept
    {
        ...
        while (!stop_.load(std::memory_order_acquire)) {
            ...
            if (next + period < clock::now()) {
                // A synthetic device cannot truthfully claim callbacks that the
                // host never executed. Drop accumulated lateness equally on all
                // platforms and continue from the next real callback boundary;
                // frame-driven tests then take longer under load instead of
                // silently fast-forwarding their audio evidence.
                next = clock::now();
            }
        }
    }
