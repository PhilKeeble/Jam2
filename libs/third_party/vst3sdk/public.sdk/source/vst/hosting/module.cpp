...
PluginFactory::ClassInfos PluginFactory::classInfos () const noexcept
{
	...
	// Some shipping plug-ins publish their classes shortly after returning a
	// factory. A single zero count made those modules disappear from Jam2's
	// isolated scan and could fail again when the runtime worker reopened them.
	// Scanning and worker initialization are non-real-time boundaries, so allow
	// a bounded half-second for that registration to settle.
	for (uint32_t attempt = 0; count == 0 && attempt < 100; ++attempt)
	{
	...
	{
		auto previousSize = classes.size ();
		...
		// A malformed factory may advertise a class but reject every metadata
		// interface. The upstream unconditional back() was undefined in that
		// case; skip the unusable class safely.
		if (classes.size () == previousSize)
			continue;
