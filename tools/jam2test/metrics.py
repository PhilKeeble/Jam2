...
def summarize_csv(path, assessment_elapsed_ms=None):
    ...
    audio_applied_playback_ratios = [
        to_float(period, "audio_applied_playback_ratio")
        for period in assessment_periods
    ]
    audio_playback_ratio_ramping_rows = sum(
        1 for period in assessment_periods
        if str(period.get("audio_playback_ratio_ramping", "")).lower()
        in {"yes", "true", "1", "on"}
    )
    ...
    return {
        ...
        "audio_device_id": to_int(row, "audio_device_id"),
        "device_backend": row.get("device_backend", ""),
        "device_name": row.get("device_name", ""),
        "device_driver_id": row.get("device_driver_id", ""),
        "device_path": row.get("device_path", ""),
        "backend_sample_format": row.get("backend_sample_format", ""),
        ...
        "capture_ring_frames": to_float(row, "capture_ring_frames"),
        "playback_ring_frames": to_float(row, "playback_ring_frames"),
        "playback_prefill_frames": to_float(row, "playback_prefill_frames"),
        "playback_max_frames": to_float(row, "playback_max_frames"),
        "elapsed_s": elapsed_s,
        ...
        "audio_applied_playback_ratio_observed_min": min(
            audio_applied_playback_ratios, default=0.0),
        "audio_applied_playback_ratio_observed_max": max(
            audio_applied_playback_ratios, default=0.0),
        "audio_playback_ratio_ramping_rows": audio_playback_ratio_ramping_rows,
        "adaptive_playback_ratio_ramp_ms": to_float(
            row, "adaptive_playback_ratio_ramp_ms"),
        ...
        "os_priority_requested": row.get("os_priority_requested", ""),
        "os_platform": row.get("os_platform", ""),
        "os_cpu_count": to_int(row, "os_cpu_count"),
        "os_process_priority_active": row.get("os_process_priority_active", ""),
        "os_thread_priority_active": row.get("os_thread_priority_active", ""),
        "os_realtime_requested": row.get("os_realtime_requested", ""),
        "os_realtime_active": row.get("os_realtime_active", ""),
        "os_realtime_error": row.get("os_realtime_error", ""),
...
def combined_summary(server_csv, client_csv, assessment_elapsed_ms=None):
    ...
    combined = {
        ...
        "jitter_avg_ms_max": max((side.get("jitter_avg_ms", 0.0) for side in sides), default=0.0),
        "jitter_max_ms": max((side.get("jitter_max_ms", 0.0) for side in sides), default=0.0),
        "rtt_avg_ms_max": max((side.get("rtt_avg_ms", 0.0) for side in sides), default=0.0),
        "rtt_max_ms": max((side.get("rtt_max_ms", 0.0) for side in sides), default=0.0),
        "playback_depth_avg_ms_max": max(
            (side.get("playback_depth_avg_ms", 0.0) for side in sides), default=0.0),
        "playback_depth_max_ms_max": max(
            (side.get("playback_depth_max_ms", 0.0) for side in sides), default=0.0),
        ...
        "audio_applied_playback_ratio_observed_max": max(
            (side.get("audio_applied_playback_ratio_observed_max", 0.0) for side in sides), default=0.0),
        "audio_playback_ratio_ramping_rows_total": sum(
            (side.get("audio_playback_ratio_ramping_rows", 0.0) for side in sides), 0.0),
...
def normalized_pair_summary(
    ...
    """Expose two-peer reduction under stable machine/peer identities.

    `combined_summary` is the internal numeric reducer. Benchmark artifacts
    publish this normalized shape and do not make server/client roles their
    data model.
    """
