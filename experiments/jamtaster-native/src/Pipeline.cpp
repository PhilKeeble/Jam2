...
PipelineResult runPipeline(const PipelineOptions& options,PipelineProgress progress)
{
    ...
    if(!options.force&&std::filesystem::is_regular_file(result.analysisReport)&&std::filesystem::is_regular_file(finalJamjar)) {
        result.cached=true;if(progress)progress(100,"Using saved JamTaster analysis");return result;
    }
    ...
    timings["separation_seconds"]=elapsed(started);Json stemsReport=Json::object();stemsReport["format"]="jamtaster-stems-v1";stemsReport["input_path"]=options.input.generic_string();stemsReport["source_sha256"]=sourceHash;stemsReport["cached"]=stemCache;
    Json stemValues=Json::object();for(const auto&[name,path]:stemPaths)stemValues[name]=path.generic_string();stemsReport["stems"]=stemValues;writeText(sourceRoot/"stems.json",stemsReport.dump(2));checkpoint("separation",18);
    ...
    timings["beat_tracking_seconds"]=elapsed(started);Json tempo=Json::object();tempo["format"]="jamtaster-tempo-v1";tempo["bpm"]=result.analysis.detectedBpm;tempo["project_bpm"]=result.analysis.bpm;tempo["beats_per_bar"]=result.analysis.beatsPerBar;tempo["beats"]=jsonNumbers(result.analysis.beats);tempo["downbeats"]=jsonNumbers(result.analysis.downbeats);writeText(sourceRoot/"tempo.json",tempo.dump(2));checkpoint("beat_tracking",27);
    ...
    if(options.exportJamJar){started=Clock::now();result.jamjar=exportJamJar(sourceRoot/"converted",options.name,sourceHash,stemPaths,result.analysis,result.sections,options.arrangementLoop,options.timeStretch);result.songRoot=result.jamjar.parent_path();timings["jamjar_export_seconds"]=elapsed(started);checkpoint("jamjar_export",97);}
    timings["pipeline_seconds_before_report"]=elapsed(pipelineStarted);timings["audio_duration_seconds"]=duration;
    writeText(result.analysisReport,analysisJson(result.analysis,result.sections,timings,options.input,sourceHash).dump(2));
    Json manifest=Json::object();manifest["format"]="jamtaster-manifest-v1";manifest["source_sha256"]=sourceHash;manifest["analysis"]=result.analysisReport.filename().generic_string();manifest["jamjar"]=result.jamjar.generic_string();manifest["song_root"]=result.songRoot.generic_string();writeText(sourceRoot/"manifest.json",manifest.dump(2));
